            ...
            while true do
                ...
                if _time == -1 then
                    ...
                    if _result == 0 then
                        ...
                        local _attack =
                            math.modf(
                            _playerA.AttackVal.Value - math.randomFloat(0, _playerA.AttackVal.Value * (1 / 3))
                        )
                        NetUtil.Fire_C("MBattleEvent", _playerB, "BeHit", _attack, _playerA)

                        local _attack2 =
                            math.modf(
                            _playerB.AttackVal.Value - math.randomFloat(0, _playerB.AttackVal.Value * (1 / 3))
                        )
                        if _isNpc then
                    ...
                    if (_playerB.HealthVal.Value <= 0) then
                        ...
                        ---发奖励的临时代码
                        NetUtil.Fire_C("UpdateCoinEvent", _playerB, 20)
                        NetUtil.Fire_C("GetItemEvent", _playerB, 5020)
                        NetUtil.Fire_C("UpdateCoinEvent", _playerA, 50)
                        NetUtil.Fire_C("GetItemEvent", _playerA, 5020)
                        _breakFlag = true
                    end
                    if (_playerA.HealthVal.Value <= 0) then
                        ...
                        ---发奖励的临时代码
                        NetUtil.Fire_C("UpdateCoinEvent", _playerB, 50)
                        NetUtil.Fire_C("GetItemEvent", _playerB, 5020)
                        NetUtil.Fire_C("UpdateCoinEvent", _playerA, 20)
                        NetUtil.Fire_C("GetItemEvent", _playerA, 5020)
                        _breakFlag = true
                    end
                    if _breakFlag then
