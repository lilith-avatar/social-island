--- This file is generated by ava-x2l.exe,
--- Don't change it manaully.
--- @copyright Lilith Games, Project Da Vinci(Avatar Team)
--- @see Official Website: https://www.projectdavinci.com/
--- @see Dev Framework: https://github.com/lilith-avatar/avatar-ava
--- @see X2L Tool: https://github.com/lilith-avatar/avatar-ava-xls2lua
--- source file: .//SoundConfig.xlsx

local SoundXls = {
    [1] = {
        ID = 1,
        Name = 'Clap',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Clap',
        Info = ''
    },
    [2] = {
        ID = 2,
        Name = 'BGM',
        IsLoop = true,
        Volume = 0.0,
        MinDistance = 20,
        MaxDistance = 50,
        Path = 'Audio/BGM_Forest',
        Info = ''
    },
    [3] = {
        ID = 3,
        Name = 'Dialogue',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Dialogue',
        Info = '对话弹出'
    },
    [4] = {
        ID = 4,
        Name = 'GetCoin',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 25,
        MaxDistance = 50,
        Path = 'Audio/GetCoin',
        Info = '吃金币的音效'
    },
    [5] = {
        ID = 5,
        Name = 'UI_Open',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/UI_Open',
        Info = '界面弹出（支付）'
    },
    [6] = {
        ID = 6,
        Name = 'Sfx_Shop_1s_Close',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Sfx_Shop_1s_Close',
        Info = '界面关闭（支付）'
    },
    [7] = {
        ID = 7,
        Name = 'UI_Buy_Confirm',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/UI_Buy_Confirm',
        Info = '点击确认（支付）'
    },
    [8] = {
        ID = 8,
        Name = 'UI_Buy_Confirm',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/UI_Buy_Confirm',
        Info = '确认支付'
    },
    [9] = {
        ID = 9,
        Name = 'Snail_CountDown',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Snail_CountDown',
        Info = '赛前预备'
    },
    [10] = {
        ID = 10,
        Name = 'Snail_Go',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Snail_Go',
        Info = '起跑'
    },
    [11] = {
        ID = 11,
        Name = 'Snail_GetMoney',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Snail_GetMoney',
        Info = '赢到钱'
    },
    [12] = {
        ID = 12,
        Name = 'Snail_LoseMoney',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Snail_LoseMoney',
        Info = '输了钱'
    },
    [13] = {
        ID = 13,
        Name = 'Catch_Animal',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 20,
        Path = 'Audio/Catch_Animal',
        Info = '成功捕捉'
    },
    [14] = {
        ID = 14,
        Name = 'Sit',
        IsLoop = false,
        Volume = 35.0,
        MinDistance = 10,
        MaxDistance = 20,
        Path = 'Audio/Sit',
        Info = '坐下的音效'
    },
    [15] = {
        ID = 15,
        Name = 'Wood_Horse',
        IsLoop = true,
        Volume = 5.0,
        MinDistance = 10,
        MaxDistance = 20,
        Path = 'Audio/Wood_Horse',
        Info = '摇木马的咯吱声'
    },
    [16] = {
        ID = 16,
        Name = 'TempBGM',
        IsLoop = true,
        Volume = 60.0,
        MinDistance = 20,
        MaxDistance = 50,
        Path = 'Audio/TempBGM',
        Info = '鸟语（环境）'
    },
    [17] = {
        ID = 17,
        Name = 'FootStep_Grass_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Grass_L',
        Info = '草地左脚步声脚步声'
    },
    [18] = {
        ID = 18,
        Name = 'PlayerControlModule_Jump_Deck',
        IsLoop = false,
        Volume = 10.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/PlayerControlModule_Jump_Deck',
        Info = '跳落地'
    },
    [19] = {
        ID = 19,
        Name = 'FootStep_Water',
        IsLoop = false,
        Volume = 40.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Water',
        Info = '进入水中的声音'
    },
    [20] = {
        ID = 20,
        Name = 'WaterWalking',
        IsLoop = false,
        Volume = 10.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/WaterWalking',
        Info = '游泳声'
    },
    [21] = {
        ID = 21,
        Name = 'SwimingBg',
        IsLoop = false,
        Volume = 10.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/SwimingBg',
        Info = '游泳移动的声音'
    },
    [22] = {
        ID = 22,
        Name = 'Bounce',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Bounce',
        Info = '踩蘑菇'
    },
    [23] = {
        ID = 23,
        Name = 'Marquis_ClothChange',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 20,
        Path = 'Audio/Marquis_ClothChange',
        Info = '换外观的交互声音'
    },
    [24] = {
        ID = 24,
        Name = 'Fruit_Logic_pickup_02',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 20,
        Path = 'Audio/Fruit_Logic_pickup_02',
        Info = '拿起的交互声音'
    },
    [25] = {
        ID = 25,
        Name = 'Weapon_Common_pickup',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 20,
        Path = 'Audio/Weapon_Common_pickup',
        Info = '装备的交互声音'
    },
    [26] = {
        ID = 26,
        Name = 'Fruit_Logic_eat&boost',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 20,
        Path = 'Audio/Fruit_Logic_eat&boost',
        Info = '吃食物的交互声音'
    },
    [27] = {
        ID = 27,
        Name = 'Weapon_attack',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_attack',
        Info = '挥动近战武器（重）'
    },
    [28] = {
        ID = 28,
        Name = 'Weapon_LightSaber_attack',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_LightSaber_attack',
        Info = '挥动光剑'
    },
    [29] = {
        ID = 29,
        Name = 'Weapon_knife_attack',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_knife_attack',
        Info = '挥动近战武器（利刃）'
    },
    [30] = {
        ID = 30,
        Name = 'Weapon_Light',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_Light',
        Info = '挥动近战武器（轻）'
    },
    [31] = {
        ID = 31,
        Name = 'Weapon_LightSaber_hit',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_LightSaber_hit',
        Info = '光剑命中'
    },
    [32] = {
        ID = 32,
        Name = 'Weapon_Umbrella_hit',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_Umbrella_hit',
        Info = '命中玩家'
    },
    [33] = {
        ID = 33,
        Name = 'Weapon_Trombone_hit',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_Trombone_hit',
        Info = '命中除了树、人、动物外其他物体'
    },
    [34] = {
        ID = 34,
        Name = 'Item_unequip',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Item_unequip',
        Info = '实体和道具消失时烟雾炸开的声音'
    },
    [35] = {
        ID = 35,
        Name = 'Weapon_LightSaber_pickup',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_LightSaber_pickup',
        Info = '装备光剑'
    },
    [36] = {
        ID = 36,
        Name = 'Throw',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Throw',
        Info = '投掷'
    },
    [37] = {
        ID = 37,
        Name = 'Weapon_Sword_Hit_Wood_Dull_01',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 20,
        Path = 'Audio/Weapon_Sword_Hit_Wood_Dull_01',
        Info = '金属命中其他物体'
    },
    [38] = {
        ID = 38,
        Name = 'Snail_Winner',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Snail_Winner',
        Info = '第一名到达终点'
    },
    [39] = {
        ID = 39,
        Name = 'Snail_Winner_02',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Snail_Winner_02',
        Info = '第二名到达终点'
    },
    [40] = {
        ID = 40,
        Name = 'Snail_Loser',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Snail_Loser',
        Info = '第三名到达终点'
    },
    [41] = {
        ID = 41,
        Name = 'Snail_Loser',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Snail_Loser',
        Info = '第四名到达终点'
    },
    [42] = {
        ID = 42,
        Name = 'Trap_Place',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Trap_Place',
        Info = '放夹子'
    },
    [43] = {
        ID = 43,
        Name = 'Trap_Effect',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 30,
        Path = 'Audio/Trap_Effect',
        Info = '夹中动物'
    },
    [44] = {
        ID = 44,
        Name = 'Trap_Break',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Trap_Break',
        Info = '夹子被毁坏'
    },
    [45] = {
        ID = 45,
        Name = 'Animal_Reindeer_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Idle',
        Info = '鹿待机'
    },
    [46] = {
        ID = 46,
        Name = 'Animal_Reindeer_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Reindeer_Dead',
        Info = '鹿死亡'
    },
    [47] = {
        ID = 47,
        Name = 'Animal_Pheasant_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Pheasant_Idle',
        Info = '山鸡待机'
    },
    [48] = {
        ID = 48,
        Name = 'Animal_Pheasant_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Pheasant_Dead',
        Info = '山鸡死亡'
    },
    [49] = {
        ID = 49,
        Name = 'Animal_Boar_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Boar_Idle',
        Info = '野猪待机'
    },
    [50] = {
        ID = 50,
        Name = 'Animal_Boar_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Boar_Dead',
        Info = '野鸡死亡'
    },
    [51] = {
        ID = 51,
        Name = 'Animal_Wolf_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Wolf_Idle',
        Info = '狼待机'
    },
    [52] = {
        ID = 52,
        Name = 'Animal_Wolf_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Wolf_Dead',
        Info = '狼死亡'
    },
    [53] = {
        ID = 53,
        Name = 'Animal_Owl_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Owl_Idle',
        Info = '猫头鹰待机'
    },
    [54] = {
        ID = 54,
        Name = 'Animal_Owl_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Owl_Dead',
        Info = '猫头鹰死亡'
    },
    [55] = {
        ID = 55,
        Name = 'Weapon_Bow_draw',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Weapon_Bow_draw',
        Info = '拉弓'
    },
    [56] = {
        ID = 56,
        Name = 'Weapon_Bow_shoot',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 10,
        MaxDistance = 15,
        Path = 'Audio/Weapon_Bow_shoot',
        Info = '放箭'
    },
    [57] = {
        ID = 57,
        Name = 'Animal_Bear_Idle',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Bear_Idle',
        Info = '熊待机'
    },
    [58] = {
        ID = 58,
        Name = 'Animal_Bear_Dead',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Animal_Bear_Dead',
        Info = '熊死亡'
    },
    [59] = {
        ID = 59,
        Name = 'Rocket_Launcher',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 5,
        MaxDistance = 30,
        Path = 'Audio/Rocket_Launcher',
        Info = '弹射椅发射的音效'
    },
    [60] = {
        ID = 60,
        Name = '0.0',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = nil,
        MaxDistance = nil,
        Path = 'Audio/0',
        Info = '弹射椅故障旋转的音效'
    },
    [61] = {
        ID = 61,
        Name = '0.0',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = nil,
        MaxDistance = nil,
        Path = 'Audio/0',
        Info = '玩家失败落下时的音效'
    },
    [62] = {
        ID = 62,
        Name = '0.0',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = nil,
        MaxDistance = nil,
        Path = 'Audio/0',
        Info = '喷气椅机械音'
    },
    [63] = {
        ID = 63,
        Name = 'Sfx_ChangeStatu_5s_SlotMachine',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Sfx_ChangeStatu_5s_SlotMachine',
        Info = '猜拳倒计时'
    },
    [64] = {
        ID = 64,
        Name = 'BGM_Party',
        IsLoop = true,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/BGM_Party',
        Info = '营地里和飞碟里BGM'
    },
    [65] = {
        ID = 65,
        Name = 'BGM_Forest',
        IsLoop = true,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/BGM_Forest',
        Info = '野外探索狩猎时BGM'
    },
    [66] = {
        ID = 66,
        Name = '0.0',
        IsLoop = true,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/0',
        Info = '迷宫内BGM'
    },
    [67] = {
        ID = 67,
        Name = 'E3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/E3',
        Info = ''
    },
    [68] = {
        ID = 68,
        Name = 'F3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F3',
        Info = ''
    },
    [69] = {
        ID = 69,
        Name = 'F#3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F#3',
        Info = ''
    },
    [70] = {
        ID = 70,
        Name = 'G3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G3',
        Info = ''
    },
    [71] = {
        ID = 71,
        Name = 'G#3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G#3',
        Info = ''
    },
    [72] = {
        ID = 72,
        Name = 'A3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/A3',
        Info = ''
    },
    [73] = {
        ID = 73,
        Name = 'A#3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/A#3',
        Info = ''
    },
    [74] = {
        ID = 74,
        Name = 'B3',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/B3',
        Info = ''
    },
    [75] = {
        ID = 75,
        Name = 'C4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/C4',
        Info = ''
    },
    [76] = {
        ID = 76,
        Name = 'C#4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/C#4',
        Info = ''
    },
    [77] = {
        ID = 77,
        Name = 'D4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/D4',
        Info = ''
    },
    [78] = {
        ID = 78,
        Name = 'D#4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/D#4',
        Info = ''
    },
    [79] = {
        ID = 79,
        Name = 'E4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/E4',
        Info = ''
    },
    [80] = {
        ID = 80,
        Name = 'F4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F4',
        Info = ''
    },
    [81] = {
        ID = 81,
        Name = 'F#4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F#4',
        Info = ''
    },
    [82] = {
        ID = 82,
        Name = 'G4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G4',
        Info = ''
    },
    [83] = {
        ID = 83,
        Name = 'G#4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G#4',
        Info = ''
    },
    [84] = {
        ID = 84,
        Name = 'A4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/A4',
        Info = ''
    },
    [85] = {
        ID = 85,
        Name = 'A#4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/A#4',
        Info = ''
    },
    [86] = {
        ID = 86,
        Name = 'B4',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/B4',
        Info = ''
    },
    [87] = {
        ID = 87,
        Name = 'C5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/C5',
        Info = ''
    },
    [88] = {
        ID = 88,
        Name = 'C#5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/C#5',
        Info = ''
    },
    [89] = {
        ID = 89,
        Name = 'D5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/D5',
        Info = ''
    },
    [90] = {
        ID = 90,
        Name = 'D#5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/D#5',
        Info = ''
    },
    [91] = {
        ID = 91,
        Name = 'E5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/E5',
        Info = ''
    },
    [92] = {
        ID = 92,
        Name = 'F5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F5',
        Info = ''
    },
    [93] = {
        ID = 93,
        Name = 'F#5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/F#5',
        Info = ''
    },
    [94] = {
        ID = 94,
        Name = 'G5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G5',
        Info = ''
    },
    [95] = {
        ID = 95,
        Name = 'G#5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/G#5',
        Info = ''
    },
    [96] = {
        ID = 96,
        Name = 'A5',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/A5',
        Info = ''
    },
    [97] = {
        ID = 97,
        Name = 'Swing09',
        IsLoop = true,
        Volume = 5.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Swing09',
        Info = ''
    },
    [98] = {
        ID = 98,
        Name = 'undertale',
        IsLoop = true,
        Volume = 5.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/undertale',
        Info = ''
    },
    [99] = {
        ID = 99,
        Name = 'creep',
        IsLoop = true,
        Volume = 5.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/creep',
        Info = ''
    },
    [100] = {
        ID = 100,
        Name = 'Night',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Night',
        Info = ''
    },
    [101] = {
        ID = 101,
        Name = 'UI_Change_Select',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/UI_Change_Select',
        Info = '切换商品选中状态'
    },
    [102] = {
        ID = 102,
        Name = 'Bonfire',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Bonfire',
        Info = '点火'
    },
    [103] = {
        ID = 103,
        Name = 'Enter_Tent',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Enter_Tent',
        Info = '进入帐篷'
    },
    [104] = {
        ID = 104,
        Name = 'Radio_Switch',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Radio_Switch',
        Info = '打开收音机'
    },
    [105] = {
        ID = 105,
        Name = 'Statue_Eye',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Statue_Eye',
        Info = '点亮石像眼睛'
    },
    [106] = {
        ID = 106,
        Name = 'Statue_Eye_Hold',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Statue_Eye_Hold',
        Info = '石像眼睛发光时一直播放'
    },
    [107] = {
        ID = 107,
        Name = 'Pearl',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Pearl',
        Info = '与珍珠交互时的声音'
    },
    [108] = {
        ID = 108,
        Name = 'Transmit',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Transmit',
        Info = '飞碟传送门触发'
    },
    [109] = {
        ID = 109,
        Name = 'Weapon_GetBuff',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Weapon_GetBuff',
        Info = '获得状态'
    },
    [110] = {
        ID = 110,
        Name = 'GetItem',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/GetItem',
        Info = '获得物品'
    },
    [111] = {
        ID = 111,
        Name = 'Exchange',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Exchange',
        Info = '变卖物品'
    },
    [112] = {
        ID = 112,
        Name = 'Arch_Hit_Target',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Arch_Hit_Target',
        Info = ''
    },
    [113] = {
        ID = 113,
        Name = 'Arch_Hit_People',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Arch_Hit_People',
        Info = ''
    },
    [114] = {
        ID = 114,
        Name = 'Arch_Hit_Apple',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Arch_Hit_Apple',
        Info = ''
    },
    [115] = {
        ID = 115,
        Name = 'Arch_Hit_Chest',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Arch_Hit_Chest',
        Info = ''
    },
    [116] = {
        ID = 116,
        Name = 'Statue_Break',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Statue_Break',
        Info = '石像破碎'
    },
    [117] = {
        ID = 117,
        Name = 'Chest_Open',
        IsLoop = false,
        Volume = 30.0,
        MinDistance = 13,
        MaxDistance = 30,
        Path = 'Audio/Chest_Open',
        Info = '打开宝箱'
    },
    [118] = {
        ID = 118,
        Name = 'FootStep_Grass_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Grass_R',
        Info = '草地右脚步声'
    },
    [119] = {
        ID = 119,
        Name = 'Chest_Fall',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 30,
        Path = 'Audio/Throw',
        Info = '宝箱掉下'
    },
    [120] = {
        ID = 120,
        Name = 'Apple_Fall',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 15,
        MaxDistance = 30,
        Path = 'Audio/Throw',
        Info = '苹果掉下'
    },
    [121] = {
        ID = 121,
        Name = 'FootStep_Cloth_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Cloth_L',
        Info = '布左脚步声'
    },
    [122] = {
        ID = 122,
        Name = 'FootStep_Cloth_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Cloth_R',
        Info = '布右脚步声'
    },
    [123] = {
        ID = 123,
        Name = 'FootStep_Stone_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Stone_L',
        Info = '踩到石头上的声音'
    },
    [124] = {
        ID = 124,
        Name = 'FootStep_Stone_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Stone_R',
        Info = '踩到石头上的声音'
    },
    [125] = {
        ID = 125,
        Name = 'FootStep_Wood_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Wood_L',
        Info = '踩到木头上的声音'
    },
    [126] = {
        ID = 126,
        Name = 'FootStep_Wood_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Wood_R',
        Info = '踩到木头上的声音'
    },
    [127] = {
        ID = 127,
        Name = 'FootStep_Metal_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Metal_L',
        Info = '踩到UFO地板的声音'
    },
    [128] = {
        ID = 128,
        Name = 'FootStep_Metal_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Metal_R',
        Info = '踩到UFO地板的声音'
    },
    [129] = {
        ID = 129,
        Name = 'FootStep_Cloud_L',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Cloud_L',
        Info = '踩到云朵上的声音'
    },
    [130] = {
        ID = 130,
        Name = 'FootStep_Cloud_R',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/FootStep_Cloud_R',
        Info = '踩到云朵上的声音'
    },
    [131] = {
        ID = 131,
        Name = 'Football_Kick',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/Football_Kick',
        Info = '踢球声'
    },
    [132] = {
        ID = 132,
        Name = 'Football_Hit',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/Football_Hit',
        Info = '球碰到其他物体声'
    },
    [133] = {
        ID = 133,
        Name = 'Football_Fresh',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/Football_Fresh',
        Info = '球出现（箭靶出现）'
    },
    [134] = {
        ID = 134,
        Name = 'Football_Goal',
        IsLoop = false,
        Volume = 60.0,
        MinDistance = 7,
        MaxDistance = 15,
        Path = 'Audio/Football_Goal',
        Info = '球进门'
    },
    [135] = {
        ID = 135,
        Name = 'Cooking',
        IsLoop = false,
        Volume = 80.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/Cooking',
        Info = '做饭过程中的声音'
    },
    [136] = {
        ID = 136,
        Name = 'CookDon',
        IsLoop = false,
        Volume = 80.0,
        MinDistance = 140,
        MaxDistance = 200,
        Path = 'Audio/CookDon',
        Info = '做饭完成的声音'
    }
}

return SoundXls
