...
function PlantFlower:EventBind()
    ...
    this.PlantGUI.PlantButton1.OnClick:Connect(
        function()
            if CanPlantFlower() then
                OnPlant(nil, 1)
            end
        end
    )
    this.PlantGUI.PlantButton2.OnClick:Connect(
        function()
            if CanPlantFlower() then
                OnPlant(nil, 2)
            end
    ...
    this.PlantGUI.PlantButton3.OnClick:Connect(
        function()
            if CanPlantFlower() then
                OnPlant(nil, 3)
            end
...
function CheckFlowerLandBegin(_hitObj, _hitPoint, hitNormal)
    ...
    if _hitObj.Name == "FlowerLand" then
        NetUtil.Fire_C("ChangeMiniGameUIEvent", localPlayer, 7)
...
function OnPlant(_longData, _level)
    invoke(
        function()
            local _pos =
                Vector3(localPlayer.Position.x, -100000, localPlayer.Position.z) + localPlayer.Forward +
                math.randomOnUnitSphere() * 0.5
            local _flower =
...
function OnWater()
    if true and waterCheckTime > WATER_CD then --TODO:判断是否手持花洒
        ...
        this.PlantGUI:SetActive(false)
        local _waterFx =
            world:CreateInstance("FX_WATER", "fx_water", world.FlowerLand, Vector3.Zero, EulerDegree(0, 0, 0))
