local BowFallState = class('BowFallState', PlayerActState)

function BowFallState:initialize(_controller, _stateName)
...
function BowFallState:InitData()
    self:AddTransition(
        'ToIdleState',
...
function BowFallState:OnEnter()
    PlayerActState.OnEnter(self)
    PlayerAnimMgr:Play(self.stateName, 0, 1, 0.1, 0.1, true, true, 1)
end

function BowFallState:OnUpdate(dt)
    PlayerActState.OnUpdate(self, dt)
    self:Move()
    self:SpeedMonitor()
end

function BowFallState:OnLeave()
