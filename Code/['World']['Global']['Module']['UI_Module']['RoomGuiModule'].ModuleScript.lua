...
function RoomGui:CreateGui()
    ...
    self.settingGui.ImgSettingBG.BtnLock.OnClick:Connect(
        function()
    ...
    self.settingGui.ImgSettingBG.BtnCancle.OnClick:Connect(
        function()
    ...
    self.gameListGui.ImgPopsBG.BtnSure.OnClick:Connect(
        function()
    ...
    self.gameListGui.ImgPopsBG.BtnBack.OnClick:Connect(
        function()
            self.gameListGui:SetActive(false)
            ResetGameList()
            isCreating = false
    ...
    self.settingGui.ImgSettingBG.ImgSlider.BtnAdd.OnClick:Connect(
        function()
    ...
    self.settingGui.ImgSettingBG.ImgSlider.BtnMinus.OnClick:Connect(
        function()
...
function RoomGui:StartCreateRoom(_gameId)
    ...
    if
        FsmMgr.playerActCtrl.curState.stateName == 'IdleState' or
            FsmMgr.playerActCtrl.curState.stateName == 'BowIdleState'
     then
        if localPlayer.Position.x > 2000 then
            NetUtil.Fire_C(
                'InsertInfoEvent',
                localPlayer,
                LanguageUtil.GetText(Config.GuiText.BoardGame_1.Txt),
                3,
                true
            )
            return
        ...
        invoke(
            function()
                LocalRooms:TryChangeRoom(_gameId)
            end,
            0.5
        )
        invoke(
...
function InitGameList()
    ...
    for k, v in pairs(Config.Game) do
        ...
        i = i + 1
        local info = world:CreateInstance('GameInfo', 'GameInfo', self.gameListGui.ImgPopsBG.PnlGameList)
        local x = (i - 1) % 4
        local y = (i - x - 1) / 4
