...
function WeaponBase:initialize(_data)
...
function WeaponBase:GetAttackData()
    return {
        ...
        hitAddBuff = self.config.HitAddBuff,
        hitRemoveBuff = self.config.HitRemoveBuff
    }
end

--播放使用音效
function WeaponBase:PlayUseSound()
    NetUtil.Fire_C("PlayEffectEvent", self.config.UseSoundID)
end
